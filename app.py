...
def generate_mock_ai_response(question):
    ...
    if question_type == 'what_is':
        topic = question_lower.replace('what is', '').replace('what are', '').replace("what's", '').strip()
        responses = [
        ...
        topic = question_lower.replace('explain', '').replace('tell me about', '').strip()
    ...
    elif any(word in question_lower for word in ['python', 'programming', 'code', 'coding']):
